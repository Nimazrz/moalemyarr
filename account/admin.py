...
class CustomUserAdmin(BaseUserAdmin):
    list_display = (
        ...
        'is_staff',
        'is_active',
        'is_question_designer',
        'is_student',
        'date_joined'
    )
    search_fields = ('code_meli', 'username', 'first_name', 'last_name', 'email')
    list_filter = ('is_active', 'is_staff', 'is_question_designer', 'is_student')
    fieldsets = (
        (None, {'fields': ('code_meli', 'password')}),
        (_('Personal info'), {
            'fields': (
                'username', 'first_name', 'last_name', 'email', 'phone', 'address', 'province', 'city', 'county',
                'landline_phone', 'bio', 'profile'
            )
        }),
        (_('Permissions'), {
            'fields': (
                'is_active', 'is_staff', 'is_superuser', 'is_question_designer', 'is_student', 'groups',
                'user_permissions'
            )
        }),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('code_meli', 'password1', 'password2', 'username', 'first_name', 'last_name', 'is_question_designer', 'is_student'),
        }),
    )
    ordering = ('code_meli',)
